...
Эта часть про то, как система ведёт себя под нагрузкой и при сбоях: HPA/VPA/KEDA, storage и расширение volume, размещение по нодам/зонам (affinity/taints/spread) и контейнерные паттерны (init/sidecar/ephemeral) как практические инструменты, а не модные слова.
...
HPA держит сервис живым при росте нагрузки, если метрики корректные.
