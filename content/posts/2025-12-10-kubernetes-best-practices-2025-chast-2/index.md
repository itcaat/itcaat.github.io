...
Хотя современные приложения обычно используют DNS и более гибкие подходы к service discovery, сценарии, где порядок создания Service → Deployment критичен, все еще существуют. Особенно:

- Унаследованные системы без модернизации
- Сложные Stateful приложения с жесткими зависимостями
- Сценарии восстановления/миграции кластеров
- Мультитенантные среды, где каждый tenant имеет свой набор сервисов

Лучшая практика: Всегда создавайте Services перед Deployments, даже если ваше приложение использует DNS. Это дешевая страховка от тонких и сложно отлаживаемых проблем.
