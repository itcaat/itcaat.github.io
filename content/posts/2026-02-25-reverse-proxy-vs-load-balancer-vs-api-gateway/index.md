...
Термины "Reverse Proxy", "Load Balancer" и "API Gateway" – давайте разберемся в чем разница с точки зрения реальной эксплуатации и закроем этот вопрос навсегда.

**Reverse Proxy** cидит перед вашим бэкендом и скрывает его от мира. Зачем? Да потому что тогда вашему приложению придется заниматься тем, чем ему не надо бы заниматься. Например, терминация SSL, отдача статики, включение компрессии gzip и тд. Прокси берет эту "грязную работу" на себя.
...
**Самое главное – это не разные сущности.** В основе всего лежит концепция Reverse Proxy.
...
Инструменты вроде Nginx, HAProxy или Traefik могут брать на себя разные роли в зависимости от того, как вы их настроите. И к выбору надо подходить исходя из требований к архитектуре. Тот же Nginx может завести вас очень далеко… =)
