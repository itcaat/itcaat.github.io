---
title: "Лучшие практики конфигурирования Kubernetes в 2025"
date: 2025-12-10T13:39:35+03:00
description: "Конфигурация в Kubernetes — это как игра в русскую рулетку с YAML-файлами. Один лишний пробел вместо таба, забытая буква в версии API, или случайно удалённая метка — и вуаля, ваше приложение лежит мёртвым грузом в продакшене, а вы судорожно пытаетесь понять в чем дело. "
thumbnail: "images/image.png"
tags: [sre, kubernetes]
draft: true
---

Конфигурация в Kubernetes — это как игра в русскую рулетку с YAML-файлами. Один лишний пробел вместо таба, забытая буква в версии API, или случайно удалённая метка — и вуаля, ваше приложение лежит мёртвым грузом в продакшене, а вы судорожно пытаетесь понять в чем дело. 

![Chaos vs Stability](images/image.png)

В этой статье я для вас собрал проверенные в боевых условиях практики, которые помогут вам спать спокойно (или хотя бы реже просыпаться от звонков on-call). Неважно, новичок вы, героически разворачивающий свой первый Pod, или бывалый кластер-администратор с тысячей yards stare — эти советы пригодятся всем.

## 📚 Основы

### Общие лучшие практики конфигурирования Kubernetes
- [1. Всегда используйте актуальную стабильную версию API](#1-всегда-используйте-актуальную-стабильную-версию-api)
- [2. Храните всю конфигурацию в системе контроля версий](#2-храните-всю-конфигурацию-в-системе-контроля-версий)
- [3. Соблюдайте принцип DRY с Kustomize или Helm](#3-соблюдайте-принцип-dry-с-kustomize-или-helm)
- [4. Используйте Namespaces для логического разделения](#4-используйте-namespaces-для-логического-разделения)

### Стандарты конфигурирования YAML
- [5. Пишите конфигурацию в YAML, а не JSON](#5-пишите-конфигурацию-в-yaml-а-не-json)
- [6. Правильно обрабатывайте булевы значения в YAML](#6-правильно-обрабатывайте-булевы-значения-в-yaml)
- [7. Держите манифесты минимальными](#7-держите-манифесты-минимальными)
- [8. Группируйте связанные объекты в одном файле](#8-группируйте-связанные-объекты-в-одном-файле)

## 🚀 Workloads и Развертывание

### Лучшие практики конфигурирования Pod и Workloads
- [9. Никогда не используйте голые Pod'ы в продакшене](#9-никогда-не-используйте-голые-podы-в-продакшене)
- [10. Используйте Deployments для stateless приложений](#10-используйте-deployments-для-stateless-приложений)
- [11. Используйте Jobs для одноразовых задач](#11-используйте-jobs-для-одноразовых-задач)
- [12. Всегда настраивайте проверки работоспособности](#12-всегда-настраивайте-проверки-работоспособности)

### Стратегии конфигурирования Deployment
- [13. Правильно настраивайте Rolling Updates](#13-правильно-настраивайте-rolling-updates)
- [14. Используйте Pod Disruption Budgets (PDB)](#14-используйте-pod-disruption-budgets-pdb)
- [15. Реализуйте Pod Anti-Affinity для высокой доступности](#15-реализуйте-pod-anti-affinity-для-высокой-доступности)

## 🌐 Сети и Сервисы

### Конфигурация Service и сетей
- [16. Создавайте Services перед зависимыми Workloads](#16-создавайте-services-перед-зависимыми-workloads)
- [17. Используйте DNS для Service Discovery](#17-используйте-dns-для-service-discovery)
- [18. Избегайте hostPort и hostNetwork](#18-избегайте-hostport-и-hostnetwork)
- [19. Используйте Headless Services для StatefulSet Discovery](#19-используйте-headless-services-для-statefulset-discovery)

### Ingress контроллеры и внешний доступ
- [61. Настройте Nginx Ingress Controller](#61-настройте-nginx-ingress-controller)
- [62. Автоматический TLS с cert-manager](#62-автоматический-tls-с-cert-manager)
- [63. Rate Limiting и защита от DDoS](#63-rate-limiting-и-защита-от-ddos)

## 🏷️ Метки и Конфигурация

### Метки, селекторы и аннотации
- [20. Используйте семантические метки последовательно](#20-используйте-семантические-метки-последовательно)
- [21. Стратегии селекции по меткам](#21-стратегии-селекции-по-меткам)
- [22. Используйте аннотации для метаданных](#22-используйте-аннотации-для-метаданных)
- [23. Отладка с использованием манипуляции метками](#23-отладка-с-использованием-манипуляции-метками)

### Управление ConfigMaps и Secrets
- [24. Используйте ConfigMaps для нечувствительной конфигурации](#24-используйте-configmaps-для-нечувствительной-конфигурации)
- [25. Безопасно работайте с Secrets](#25-безопасно-работайте-с-secrets)

### Лучшие практики переменных окружения
- [79. Понимание приоритета переменных окружения](#79-понимание-приоритета-переменных-окружения)
- [80. Используйте неизменяемые ConfigMaps и Secrets](#80-используйте-неизменяемые-configmaps-и-secrets)
- [81. Триггерите перезапуск Pod при изменении ConfigMap](#81-триггерите-перезапуск-pod-при-изменении-configmap)

## 💾 Ресурсы и Хранилище

### Управление ресурсами и лимиты
- [26. Всегда устанавливайте запросы и лимиты ресурсов](#26-всегда-устанавливайте-запросы-и-лимиты-ресурсов)
- [27. Используйте ResourceQuotas для лимитов namespace](#27-используйте-resourcequotas-для-лимитов-namespace)
- [28. Реализуйте LimitRanges для значений по умолчанию](#28-реализуйте-limitranges-для-значений-по-умолчанию)

### Классы качества обслуживания (QoS)
- [67. Понимание QoS классов](#67-понимание-qos-классов)

### Лучшие практики хранилища
- [46. Используйте PersistentVolumes для stateful приложений](#46-используйте-persistentvolumes-для-stateful-приложений)
- [47. Определите StorageClasses для разных уровней производительности](#47-определите-storageclasses-для-разных-уровней-производительности)
- [48. Включите расширение Volume](#48-включите-расширение-volume)

## 🔒 Безопасность

### Лучшие практики конфигурирования безопасности
- [29. Запускайте контейнеры не от root](#29-запускайте-контейнеры-не-от-root)
- [30. Используйте Network Policies](#30-используйте-network-policies)
- [31. Используйте Pod Security Standards (PSS) вместо устаревших PSP](#31-используйте-pod-security-standards-pss-вместо-устаревших-psp)

### RBAC и Service Accounts
- [64. Следуйте принципу минимальных привилегий](#64-следуйте-принципу-минимальных-привилегий)
- [65. Отключите автомонтирование Service Account по умолчанию](#65-отключите-автомонтирование-service-account-по-умолчанию)
- [66. Включите аудит логирование](#66-включите-аудит-логирование)

### Admission Controllers и применение политик
- [85. Используйте OPA Gatekeeper для применения политик](#85-используйте-opa-gatekeeper-для-применения-политик)
- [86. Реализуйте Validating Webhooks](#86-реализуйте-validating-webhooks)
- [87. Используйте Mutating Webhooks для автоконфигурации](#87-используйте-mutating-webhooks-для-автоконфигурации)
- [88. Альтернатива: Kyverno для простых политик](#88-альтернатива-kyverno-для-простых-политик)

## 🖼️ Образы и Контейнеры

### Лучшие практики управления образами
- [38. Используйте дайджесты образов для продакшена](#38-используйте-дайджесты-образов-для-продакшена)
- [39. Правильно настраивайте imagePullPolicy](#39-правильно-настраивайте-imagepullpolicy)
- [40. Используйте аутентификацию в приватных реестрах](#40-используйте-аутентификацию-в-приватных-реестрах)
- [41. Сканируйте образы на уязвимости](#41-сканируйте-образы-на-уязвимости)

### Паттерны контейнеров и лучшие практики
- [53. Используйте Init Containers для задач настройки](#53-используйте-init-containers-для-задач-настройки)
- [54. Реализуйте паттерн Sidecar](#54-реализуйте-паттерн-sidecar)
- [55. Используйте Ephemeral Containers для отладки](#55-используйте-ephemeral-containers-для-отладки)

### Глубокое погружение в Lifecycle Hooks
- [82. Используйте postStart Hooks для инициализации](#82-используйте-poststart-hooks-для-инициализации)
- [83. Продвинутые паттерны preStop Hook](#83-продвинутые-паттерны-prestop-hook)
- [84. Комбинируйте Hooks с Probes](#84-комбинируйте-hooks-с-probes)

## 🔄 Жизненный Цикл

### Graceful Shutdown и управление жизненным циклом
- [35. Настройте правильный период завершения](#35-настройте-правильный-период-завершения)
- [36. Правильно обрабатывайте SIGTERM в приложении](#36-правильно-обрабатывайте-sigterm-в-приложении)
- [37. Реализуйте дренаж соединений](#37-реализуйте-дренаж-соединений)

### Расширенные проверки работоспособности
- [93. Настройте проверки работоспособности для разных типов приложений](#93-настройте-проверки-работоспособности-для-разных-типов-приложений)
- [94. Реализуйте Startup Probes для медленных приложений](#94-реализуйте-startup-probes-для-медленных-приложений)
- [95. Лучшие практики проверок работоспособности](#95-лучшие-практики-проверок-работоспособности)

## 📈 Масштабирование

### Стратегии автомасштабирования
- [42. Настройте Horizontal Pod Autoscaler (HPA)](#42-настройте-horizontal-pod-autoscaler-hpa)
- [43. Используйте кастомные метрики для HPA](#43-используйте-кастомные-метрики-для-hpa)
- [44. Рассмотрите Vertical Pod Autoscaler (VPA)](#44-рассмотрите-vertical-pod-autoscaler-vpa)
- [45. Используйте KEDA для event-driven автомасштабирования](#45-используйте-keda-для-event-driven-автомасштабирования)

### Продвинутые практики планирования
- [49. Используйте Node Affinity для размещения workloads](#49-используйте-node-affinity-для-размещения-workloads)
- [50. Реализуйте Taints и Tolerations](#50-реализуйте-taints-и-tolerations)
- [51. Используйте Topology Spread Constraints (современная альтернатива)](#51-используйте-topology-spread-constraints-современная-альтернатива)
- [52. Используйте Priority Classes для критичных workloads](#52-используйте-priority-classes-для-критичных-workloads)

## 💰 Оптимизация Затрат

### Лучшие практики оптимизации затрат
- [75. Правильно подбирайте размер ваших Workloads](#75-правильно-подбирайте-размер-ваших-workloads)
- [76. Используйте Spot/Preemptible инстансы](#76-используйте-spotpreemptible-инстансы)
- [77. Реализуйте автомасштабирование кластера](#77-реализуйте-автомасштабирование-кластера)
- [78. Мониторьте и алертите на аномалии затрат](#78-мониторьте-и-алертите-на-аномалии-затрат)

## 📊 Наблюдаемость

### Наблюдаемость и мониторинг
- [32. Настройте сбор метрик Prometheus](#32-настройте-сбор-метрик-prometheus)
- [33. Реализуйте структурированное логирование](#33-реализуйте-структурированное-логирование)
- [34. Включите распределённую трассировку](#34-включите-распределённую-трассировку)

## 🔧 GitOps и Service Mesh

### GitOps и управление конфигурацией
- [56. Реализуйте GitOps с ArgoCD](#56-реализуйте-gitops-с-argocd)
- [57. Используйте FluxCD для прогрессивной доставки](#57-используйте-fluxcd-для-прогрессивной-доставки)

### Интеграция Service Mesh
- [58. Когда использовать Service Mesh](#58-когда-использовать-service-mesh)
- [59. Настройте Istio Virtual Service](#59-настройте-istio-virtual-service)
- [60. Включите mTLS с Istio](#60-включите-mtls-с-istio)

## 🛠️ Инструменты и Отладка

### kubectl советы по управлению конфигурацией
- [68. Применяйте целые директории](#68-применяйте-целые-директории)
- [69. Генерируйте манифесты из работающих ресурсов](#69-генерируйте-манифесты-из-работающих-ресурсов)
- [70. Валидируйте конфигурации перед применением](#70-валидируйте-конфигурации-перед-применением)

### Современные техники отладки
- [71. Используйте kubectl debug с Ephemeral Containers](#71-используйте-kubectl-debug-с-ephemeral-containers)
- [72. Стримите логи из нескольких Pod'ов](#72-стримите-логи-из-нескольких-podов)
- [73. Интерактивные техники отладки](#73-интерактивные-техники-отладки)
- [74. Используйте stern и kubectl плагины](#74-используйте-stern-и-kubectl-плагины)

## 🚨 Troubleshooting

### Устранение распространённых проблем
- [96. Ошибки ImagePullBackOff](#96-ошибки-imagepullbackoff)
- [97. Стратегия отладки CrashLoopBackOff](#97-стратегия-отладки-crashloopbackoff)
- [98. Pending Pod'ы - проблемы планирования](#98-pending-podы---проблемы-планирования)
- [99. Проблемы подключения к Service](#99-проблемы-подключения-к-service)
- [100. Высокое использование памяти / OOMKilled](#100-высокое-использование-памяти--oomkilled)
- [101. Проблемы разрешения DNS](#101-проблемы-разрешения-dns)

## 💾 Резервное копирование

### Резервное копирование и аварийное восстановление
- [89. Реализуйте Velero для резервных копий кластера](#89-реализуйте-velero-для-резервных-копий-кластера)
- [90. Резервные копии etcd снэпшотов](#90-резервные-копии-etcd-снэпшотов)
- [91. Создайте Runbook для аварийного восстановления](#91-создайте-runbook-для-аварийного-восстановления)
- [92. Регулярно тестируйте аварийное восстановление](#92-регулярно-тестируйте-аварийное-восстановление)

## ⚠️ Дополнительно

### Антипаттерны, которых следует избегать
- [102. Распространённые антипаттерны Kubernetes](#102-распространённые-антипаттерны-kubernetes)

### Справочники и инструменты
- [104. Справочник распространённых kubectl команд](#104-справочник-распространённых-kubectl-команд)
- [105. Полезные инструменты и расширения](#105-полезные-инструменты-и-расширения)
